...
app.get('/api/personas-mantenimiento', requireAuth,async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Error al listar personas de mantenimiento' });
  }
});
...
app.get('/reportes/localidades.xlsx', requireAuth, async (req, res) => {
  try {
    const { q } = req.query;
    ...
    if (q) {
      where.identificacion = { [Op.like]: '%' + q + '%' };
    ...
    // Crear Excel
    const wb = new ExcelJS.Workbook();
    const ws = wb.addWorksheet('Localidades');

    // Encabezados correctos según tu modelo
    ws.addRow([
      'ID',
      'Identificación',
      'Sede',
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename="Localidades.xlsx"'
    );
    ...
    console.error(err);
    res.status(500).send('Error al generar el reporte de localidades');
  }
});

// final de excel localidades


/* ===============================
 *  APIs — Ubicaciones / Responsables
 * =============================== */
app.get('/api/ubicaciones', requireAuth,async (_req, res) => {
  try {
 ...
 * =============================== */
app.get('/api/responsables-custodios', requireAuth,async (_req, res) => {
  try {
    const items = await ResponsableCustodio.findAll({ order: [['id_area', 'ASC']] });
    res.json(items.map(toDTO));   // la clave en JSON será "ResponsableCustodio" cuando venga por include, es decir, en relaciones para que no falle el frontend
  } catch (e) {
